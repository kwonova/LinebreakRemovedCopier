...
def on_press(key):
    ...
    # When LineBreakRemovedCopy is ready and command+C is pressed again
    if copyReadyState and keyboard.Key.cmd in current_pressed and keyboard.KeyCode.from_char('c') in current_pressed:
        copyReadyState = False
...
def on_release(key):
    ...
    # When command+C is initially pressed and when C is released
    if keyboard.KeyCode.from_char('c') in current_pressed and keyboard.Key.cmd in current_pressed and not copyReadyState:
        copied = True

    # When the text has been already LineBreakRemovedCopied and C is released
    if keyboard.KeyCode.from_char('c') in current_pressed and keyboard.Key.cmd in current_pressed and superCopied:
    ...
    if key in current_pressed:
        ...
        # When 'key' was left command key, initialize all states
        if keyboard.Key.cmd == key:
            superCopied = False
            copied = False
            copyReadyState = False
            current_pressed.clear()

        # When firstly copied and when left command key remains, make LineBreakRemovedCopy be ready
        if copied and keyboard.Key.cmd in current_pressed and not copyReadyState:
            copyReadyState = True
            copied = False

print ("\n"*80)
print('Welcome to Linebreak Removed Copier for Mac\n')
print('※ This tool is dedicated to lab buddies whose first language is not English.\n')
print("※ How to use: command + C C.")
print("  - When you copy the text using command + C, just click C key again (while pressing command key)")
